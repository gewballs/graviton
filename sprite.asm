	;# ===============================================================//
    ;# Crate                                                          //
    ;# ===============================================================//
    ;#Data w crate.sprite{
        $03
        $00 $00 $327E $00
        $08 $00 $727E $00
        $00 $08 $B27E $00
        $08 $08 $F27E $00
    }
    

	;# ===============================================================//
    ;# Gibs Scripts                                                   //
    ;# ===============================================================//
    ;#Data w gibs.arm0.r.script{
        $0004
		gibs.arm0.r.0 $0004
		gibs.arm0.r.1 $0004
		gibs.arm0.r.2 $0004
		gibs.arm0.r.3 $0004
    }
    ;#Data w gibs.arm0.l.script{
        $0004
		gibs.arm0.l.0 $0004
		gibs.arm0.l.1 $0004
		gibs.arm0.l.2 $0004
		gibs.arm0.l.3 $0004
    }

    ;#Data w gibs.arm1.r.script{
        $0004
		gibs.arm1.r.0 $0004
		gibs.arm1.r.1 $0004
		gibs.arm1.r.2 $0004
		gibs.arm1.r.3 $0004
    }
    ;#Data w gibs.arm1.l.script{
        $0004
		gibs.arm1.l.0 $0004
		gibs.arm1.l.1 $0004
		gibs.arm1.l.2 $0004
		gibs.arm1.l.3 $0004
    }

    ;#Data w gibs.foot.r.script{
        $0004
		gibs.foot.r.0 $0004
		gibs.foot.r.1 $0004
		gibs.foot.r.2 $0004
		gibs.foot.r.3 $0004
    }
    ;#Data w gibs.foot.l.script{
        $0004
		gibs.foot.l.0 $0004
		gibs.foot.l.1 $0004
		gibs.foot.l.2 $0004
		gibs.foot.l.3 $0004
    }

    ;#Data w gibs.head.r.script{
        $0004
		gibs.head.r.0 $0004
		gibs.head.r.1 $0004
		gibs.head.r.2 $0004
		gibs.head.r.3 $0004
    }
    ;#Data w gibs.head.l.script{
        $0004
		gibs.head.l.0 $0004
		gibs.head.l.1 $0004
		gibs.head.l.2 $0004
		gibs.head.l.3 $0004
    }
    
    ;#Data w gibs.bone.script{
        $0002
		gibs.bone.0 $0004
		gibs.bone.1 $0004
    }
    
    ;#Data w gibs.gore.r.script{
        $0002
		gibs.gore.r.0 $0004
		gibs.gore.r.1 $0004
    }
    ;#Data w gibs.gore.ri.script{
        $0002
		gibs.gore.ri.0 $0004
		gibs.gore.ri.1 $0004
    }
    ;#Data w gibs.gore.l.script{
        $0002
		gibs.gore.l.0 $0004
		gibs.gore.l.1 $0004
    }
    ;#Data w gibs.gore.li.script{
        $0002
		gibs.gore.li.0 $0004
		gibs.gore.li.1 $0004
    }


    ;# ===============================================================//
    ;# Gibs Sprites                                                   //
    ;# ===============================================================//
    ;#Data w gibs.arm0.r.0{
	    $00
		$00 $00 $3055 $00
	}
    ;#Data w gibs.arm0.r.1{
	    $00
		$00 $00 $3056 $00
	}
    ;#Data w gibs.arm0.r.2{
	    $00
		$00 $00 $F055 $00
	}
    ;#Data w gibs.arm0.r.3{
	    $00
		$00 $00 $F056 $00
	}
    ;#Data w gibs.arm0.l.0{
	    $00
		$00 $00 $7055 $00
	}
    ;#Data w gibs.arm0.l.1{
	    $00
		$00 $00 $7056 $00
	}
    ;#Data w gibs.arm0.l.2{
	    $00
		$00 $00 $B055 $00
	}
    ;#Data w gibs.arm0.l.3{
	    $00
		$00 $00 $B056 $00
	}

    ;#Data w gibs.arm1.r.0{
	    $00
		$00 $00 $3057 $00
	}
    ;#Data w gibs.arm1.r.1{
	    $00
		$00 $00 $3058 $00
	}
    ;#Data w gibs.arm1.r.2{
	    $00
		$00 $00 $F057 $00
	}
    ;#Data w gibs.arm1.r.3{
	    $00
		$00 $00 $F058 $00
	}
    ;#Data w gibs.arm1.l.0{
	    $00
		$00 $00 $7057 $00
	}
    ;#Data w gibs.arm1.l.1{
	    $00
		$00 $00 $7058 $00
	}
    ;#Data w gibs.arm1.l.2{
	    $00
		$00 $00 $B057 $00
	}
    ;#Data w gibs.arm1.l.3{
	    $00
		$00 $00 $B058 $00
	}

    ;#Data w gibs.foot.r.0{
	    $00
		$00 $00 $3059 $00
	}
    ;#Data w gibs.foot.r.1{
	    $00
		$00 $00 $305A $00
	}
    ;#Data w gibs.foot.r.2{
	    $00
		$00 $00 $F059 $00
	}
    ;#Data w gibs.foot.r.3{
	    $00
		$00 $00 $F05A $00
	}
    ;#Data w gibs.foot.l.0{
	    $00
		$00 $00 $7059 $00
	}
    ;#Data w gibs.foot.l.1{
	    $00
		$00 $00 $705A $00
	}
    ;#Data w gibs.foot.l.2{
	    $00
		$00 $00 $B059 $00
	}
    ;#Data w gibs.foot.l.3{
	    $00
		$00 $00 $B05A $00
	}

    ;#Data w gibs.head.r.0{
	    $03
		$FC $F9 $305C $01
		$04 $F9 $305D $00
		$FC $01 $305E $01
		$04 $01 $305F $00
	}
    ;#Data w gibs.head.r.1{
	    $03
		$07 $FC $3060 $00
		$07 $04 $3061 $00
		$FF $FC $3062 $01
		$FF $04 $3063 $01
	}
    ;#Data w gibs.head.r.2{
	    $03
		$04 $07 $F05C $00
		$FC $07 $F05D $01
		$04 $FF $F05E $00
		$FC $FF $F05F $01
	}
    ;#Data w gibs.head.r.3{
	    $03
		$F9 $04 $F060 $01
		$F9 $FC $F061 $01
		$01 $04 $F062 $00
		$01 $FC $F063 $00
	}
    ;#Data w gibs.head.l.0{
	    $03
		$04 $F9 $705C $00
		$FC $F9 $705D $01
		$04 $01 $705E $00
		$FC $01 $705F $01
	}
    ;#Data w gibs.head.l.1{
	    $03
		$F9 $FC $7060 $01
		$F9 $04 $7061 $01
		$01 $FC $7062 $00
		$01 $04 $7063 $00
	}
    ;#Data w gibs.head.l.2{
	    $03
		$FC $07 $B05C $01
		$04 $07 $B05D $00
		$FC $FF $B05E $01
		$04 $FF $B05F $00
	}
    ;#Data w gibs.head.l.3{
	    $03
		$07 $04 $B060 $00
		$07 $FC $B061 $00
		$FF $04 $B062 $01
		$FF $FC $B063 $01
	}

    ;#Data w gibs.bone.0{
	    $00
		$00 $00 $305B $00
	}
    ;#Data w gibs.bone.1{
	    $00
		$00 $00 $B05B $00
	}

	;#Data w gibs.gore.r.0{
	    $00
		$00 $00 $3065 $00
	}
	;#Data w gibs.gore.r.1{
	    $00
		$00 $00 $3064 $00
	}
	;#Data w gibs.gore.ri.0{
	    $00
		$00 $00 $B065 $00
	}
	;#Data w gibs.gore.ri.1{
	    $00
		$00 $00 $B064 $00
	}
	;#Data w gibs.gore.l.0{
	    $00
		$00 $00 $7065 $00
	}
	;#Data w gibs.gore.l.1{
	    $00
		$00 $00 $7064 $00
	}
	;#Data w gibs.gore.li.0{
	    $00
		$00 $00 $F065 $00
	}
	;#Data w gibs.gore.li.1{
	    $00
		$00 $00 $F064 $00
	}


    ;# ===============================================================//
    ;# Body Scripts                                                   //
    ;# ===============================================================//
    ;#Data w vrtan.body.idle.r.script{
        $0004
        vrtan.body.idle.r.0 $0008
        vrtan.body.idle.r.1 $0004
        vrtan.body.idle.r.2 $001C
        vrtan.body.idle.r.1 $0004
    }
    ;#Data w vrtan.body.idle.ri.script{
        $0004
        vrtan.body.idle.ri.0 $0008
        vrtan.body.idle.ri.1 $0004
        vrtan.body.idle.ri.2 $001C
        vrtan.body.idle.ri.1 $0004
    }
    ;#Data w vrtan.body.idle.l.script{
        $0004
        vrtan.body.idle.l.0 $0008
        vrtan.body.idle.l.1 $0004
        vrtan.body.idle.l.2 $001C
        vrtan.body.idle.l.1 $0004
    }
    ;#Data w vrtan.body.idle.li.script{
        $0004
        vrtan.body.idle.li.0 $0008
        vrtan.body.idle.li.1 $0004
        vrtan.body.idle.li.2 $001C
        vrtan.body.idle.li.1 $0004
    }

    ;#Data w vrtan.body.walk.r.script{
        $0004
        vrtan.body.walk.r.0 $0008
        vrtan.body.walk.r.1 $0004
        vrtan.body.walk.r.2 $0008
        vrtan.body.walk.r.1 $0004
    }
    ;#Data w vrtan.body.walk.ri.script{
        $0004
        vrtan.body.walk.ri.0 $0008
        vrtan.body.walk.ri.1 $0004
        vrtan.body.walk.ri.2 $0008
        vrtan.body.walk.ri.1 $0004
    }
    ;#Data w vrtan.body.walk.l.script{
        $0004
        vrtan.body.walk.l.0 $0008
        vrtan.body.walk.l.1 $0004
        vrtan.body.walk.l.2 $0008
        vrtan.body.walk.l.1 $0004
    }
    ;#Data w vrtan.body.walk.li.script{
        $0004
        vrtan.body.walk.li.0 $0008
        vrtan.body.walk.li.1 $0004
        vrtan.body.walk.li.2 $0008
        vrtan.body.walk.li.1 $0004
    }

    ;#Data w vrtan.body.jump.r.script{
        $0001
        vrtan.body.jump.r.0 $0004
    }
    ;#Data w vrtan.body.jump.ri.script{
        $0001
        vrtan.body.jump.ri.0 $0004
    }
    ;#Data w vrtan.body.jump.l.script{
        $0001
        vrtan.body.jump.l.0 $0004
    }
    ;#Data w vrtan.body.jump.li.script{
        $0001
        vrtan.body.jump.li.0 $0004
    }

    ;#Data w vrtan.body.punch.r.script{
        $0003
        vrtan.body.punch.r.0 $0004
        vrtan.body.punch.r.1 $0004
        vrtan.body.punch.r.0 $0400
    }
    ;#Data w vrtan.body.punch.ri.script{
        $0003
        vrtan.body.punch.ri.0 $0004
        vrtan.body.punch.ri.1 $0004
        vrtan.body.punch.ri.0 $0400
    }
    ;#Data w vrtan.body.punch.l.script{
        $0003
        vrtan.body.punch.l.0 $0004
        vrtan.body.punch.l.1 $0004
        vrtan.body.punch.l.0 $0400
    }
    ;#Data w vrtan.body.punch.li.script{
        $0003
        vrtan.body.punch.li.0 $0004
        vrtan.body.punch.li.1 $0004
        vrtan.body.punch.li.0 $0400
    }

    ;#Data w vrtan.body.uppercut.r.script{
        $0003
        vrtan.body.uppercut.r.0 $0004
        vrtan.body.uppercut.r.1 $0004
        vrtan.body.uppercut.r.2 $0400
    }
    ;#Data w vrtan.body.uppercut.ri.script{
        $0003
        vrtan.body.uppercut.ri.0 $0004
        vrtan.body.uppercut.ri.1 $0004
        vrtan.body.uppercut.ri.2 $0400
    }
    ;#Data w vrtan.body.uppercut.l.script{
        $0003
        vrtan.body.uppercut.l.0 $0004
        vrtan.body.uppercut.l.1 $0004
        vrtan.body.uppercut.l.2 $0400
    }
    ;#Data w vrtan.body.uppercut.li.script{
        $0003
        vrtan.body.uppercut.li.0 $0004
        vrtan.body.uppercut.li.1 $0004
        vrtan.body.uppercut.li.2 $0400
    }
    
    ;#Data w vrtan.body.crush.r.script{
        $0005
        vrtan.body.crush.r.0 $0006
        vrtan.body.crush.r.1 $0006
        vrtan.body.crush.r.2 $0004
        vrtan.body.crush.l.2 $0004
        vrtan.body.crush.r.2 $0004
    }
    ;#Data w vrtan.body.crush.ri.script{
        $0005
        vrtan.body.crush.ri.0 $0006
        vrtan.body.crush.ri.1 $0006
        vrtan.body.crush.ri.2 $0004
        vrtan.body.crush.li.2 $0004
        vrtan.body.crush.ri.2 $0004
    }
    ;#Data w vrtan.body.crush.l.script{
        $0005
        vrtan.body.crush.l.0 $0006
        vrtan.body.crush.l.1 $0006
        vrtan.body.crush.l.2 $0004
        vrtan.body.crush.r.2 $0004
        vrtan.body.crush.l.2 $0004
    }
    ;#Data w vrtan.body.crush.li.script{
        $0005
        vrtan.body.crush.li.0 $0006
        vrtan.body.crush.li.1 $0006
        vrtan.body.crush.li.2 $0004
        vrtan.body.crush.ri.2 $0004
        vrtan.body.crush.li.2 $0004
    }
   

    ;# ===============================================================//
    ;# Legs Scripts                                                   //
    ;# ===============================================================//
    ;#Data w vrtan.legs.idle.r.script{
        $0001
        vrtan.legs.idle.r.0 $0004
    }
    ;#Data w vrtan.legs.idle.ri.script{
        $0001
        vrtan.legs.idle.ri.0 $0004
    }
    ;#Data w vrtan.legs.idle.l.script{
        $0001
        vrtan.legs.idle.l.0 $0004
    }
    ;#Data w vrtan.legs.idle.li.script{
        $0001
        vrtan.legs.idle.li.0 $0004
    }

    ;#Data w vrtan.legs.walk.r.script{
        $0006
        vrtan.legs.walk.r.0 $0004
        vrtan.legs.walk.r.1 $0004
        vrtan.legs.walk.r.2 $0004
        vrtan.legs.walk.r.3 $0004
        vrtan.legs.walk.r.4 $0004
        vrtan.legs.walk.r.5 $0004
    }
    ;#Data w vrtan.legs.walk.ri.script{
        $0006
        vrtan.legs.walk.ri.0 $0004
        vrtan.legs.walk.ri.1 $0004
        vrtan.legs.walk.ri.2 $0004
        vrtan.legs.walk.ri.3 $0004
        vrtan.legs.walk.ri.4 $0004
        vrtan.legs.walk.ri.5 $0004
    }
    ;#Data w vrtan.legs.walk.l.script{
        $0006
        vrtan.legs.walk.l.0 $0004
        vrtan.legs.walk.l.1 $0004
        vrtan.legs.walk.l.2 $0004
        vrtan.legs.walk.l.3 $0004
        vrtan.legs.walk.l.4 $0004
        vrtan.legs.walk.l.5 $0004
    }
    ;#Data w vrtan.legs.walk.li.script{
        $0006
        vrtan.legs.walk.li.0 $0004
        vrtan.legs.walk.li.1 $0004
        vrtan.legs.walk.li.2 $0004
        vrtan.legs.walk.li.3 $0004
        vrtan.legs.walk.li.4 $0004
        vrtan.legs.walk.li.5 $0004
    }

    ;#Data w vrtan.legs.jump.r.script{
        $0001
        vrtan.legs.jump.r.0 $0004
    }
    ;#Data w vrtan.legs.jump.ri.script{
        $0001
        vrtan.legs.jump.ri.0 $0004
    }
    ;#Data w vrtan.legs.jump.l.script{
        $0001
        vrtan.legs.jump.l.0 $0004
    }
    ;#Data w vrtan.legs.jump.li.script{
        $0001
        vrtan.legs.jump.li.0 $0004
    }

    ;#Data w vrtan.legs.punch.r.script{
        $0001
        vrtan.legs.walk.r.1 $0004
        vrtan.legs.walk.r.1 $0004
        vrtan.legs.walk.r.1 $0004
    }
    ;#Data w vrtan.legs.punch.ri.script{
        $0001
        vrtan.legs.walk.ri.1 $0004
        vrtan.legs.walk.ri.1 $0004
        vrtan.legs.walk.ri.1 $0004
    }
    ;#Data w vrtan.legs.punch.l.script{
        $0001
        vrtan.legs.walk.l.1 $0004
        vrtan.legs.walk.l.1 $0004
        vrtan.legs.walk.l.1 $0004
    }
    ;#Data w vrtan.legs.punch.li.script{
        $0001
        vrtan.legs.walk.li.1 $0004
        vrtan.legs.walk.li.1 $0004
        vrtan.legs.walk.li.1 $0004
    }
    
    
    ;# ===============================================================//
    ;# Hitbox                                                         //
    ;# ===============================================================//
    ;#Data w hit.tl.sprite{
        $00
        $00 $00 $3C7F $00
    }
    ;#Data w hit.bl.sprite{
        $00
        $00 $F8 $BC7F $00
    }
    ;#Data w hit.br.sprite{
        $00
        $F8 $F8 $FC7F $01
    }
    ;#Data w hit.tr.sprite{
        $00
        $F8 $00 $7C7F $01
    }
    
    
    ;# ===============================================================//
    ;# Body Sprites                                                   //
    ;# ===============================================================//
    ;#Data w vrtan.body.idle.r.0{
        $09
        $00 $08 $2026 $00
        $08 $08 $2027 $00
        $00 $10 $2028 $00
        $08 $10 $2029 $00
        
        $00 $00 $2002 $00
        $08 $00 $2003 $00
        $00 $08 $200A $00
        $08 $08 $200B $00
        $00 $10 $2012 $00
        $08 $10 $2013 $00
    }
    ;#Data w vrtan.body.idle.ri.0{
        $09
        $00 $15 $A026 $00
        $08 $15 $A027 $00
        $00 $0D $A028 $00
        $08 $0D $A029 $00
        
        $00 $1D $A002 $00
        $08 $1D $A003 $00
        $00 $15 $A00A $00
        $08 $15 $A00B $00
        $00 $0D $A012 $00
        $08 $0D $A013 $00
    }
    ;#Data w vrtan.body.idle.l.0{
        $09
        $06 $08 $6026 $00
        $FE $08 $6027 $01
        $06 $10 $6028 $00
        $FE $10 $6029 $01
        
        $06 $00 $6002 $00
        $FE $00 $6003 $01
        $06 $08 $600A $00
        $FE $08 $600B $01
        $06 $10 $6012 $00
        $FE $10 $6013 $01
    }
    ;#Data w vrtan.body.idle.li.0{
        $09
        $06 $15 $E026 $00
        $FE $15 $E027 $01
        $06 $0D $E028 $00
        $FE $0D $E029 $01
        
        $06 $1D $E002 $00
        $FE $1D $E003 $01
        $06 $15 $E00A $00
        $FE $15 $E00B $01
        $06 $0D $E012 $00
        $FE $0D $E013 $01
    }
    ;#Data w vrtan.body.idle.r.1{
        $09
        $00 $09 $2026 $00
        $08 $09 $2027 $00
        $00 $11 $2028 $00
        $08 $11 $2029 $00
        
        $00 $00 $2002 $00
        $08 $00 $2003 $00
        $00 $08 $200A $00
        $08 $08 $200B $00
        $00 $10 $2012 $00
        $08 $10 $2013 $00
    }
    ;#Data w vrtan.body.idle.ri.1{
        $09
        $00 $14 $A026 $00
        $08 $14 $A027 $00
        $00 $0C $A028 $00
        $08 $0C $A029 $00
        
        $00 $1D $A002 $00
        $08 $1D $A003 $00
        $00 $15 $A00A $00
        $08 $15 $A00B $00
        $00 $0D $A012 $00
        $08 $0D $A013 $00
    }
    ;#Data w vrtan.body.idle.l.1{
        $09
        $06 $09 $6026 $00
        $FE $09 $6027 $01
        $06 $11 $6028 $00
        $FE $11 $6029 $01
        
        $06 $00 $6002 $00
        $FE $00 $6003 $01
        $06 $08 $600A $00
        $FE $08 $600B $01
        $06 $10 $6012 $00
        $FE $10 $6013 $01
    }
    ;#Data w vrtan.body.idle.li.1{
        $09
        $06 $14 $E026 $00
        $FE $14 $E027 $01
        $06 $0C $E028 $00
        $FE $0C $E029 $01
        
        $06 $1D $E002 $00
        $FE $1D $E003 $01
        $06 $15 $E00A $00
        $FE $15 $E00B $01
        $06 $0D $E012 $00
        $FE $0D $E013 $01
    }
    ;#Data w vrtan.body.idle.r.2{
        $09
        $00 $08 $202A $00
        $08 $08 $202B $00
        $FF $10 $202C $01
        $07 $10 $202D $00
        
        $00 $00 $2002 $00
        $08 $00 $2003 $00
        $00 $08 $200A $00
        $08 $08 $200B $00
        $00 $10 $2012 $00
        $08 $10 $2013 $00
    }
    ;#Data w vrtan.body.idle.ri.2{
        $09
        $00 $15 $A02A $00
        $08 $15 $A02B $00
        $FF $0D $A02C $01
        $07 $0D $A02D $00
        
        $00 $1D $A002 $00
        $08 $1D $A003 $00
        $00 $15 $A00A $00
        $08 $15 $A00B $00
        $00 $0D $A012 $00
        $08 $0D $A013 $00
    }
    ;#Data w vrtan.body.idle.l.2{
        $09
        $06 $08 $602A $00
        $FE $08 $602B $01
        $07 $10 $602C $00
        $FF $10 $602D $01
        
        $06 $00 $6002 $00
        $FE $00 $6003 $01
        $06 $08 $600A $00
        $FE $08 $600B $01
        $06 $10 $6012 $00
        $FE $10 $6013 $01
    }
    ;#Data w vrtan.body.idle.li.2{
        $09
        $06 $15 $E02A $00
        $FE $15 $E02B $01
        $07 $0D $E02C $00
        $FF $0D $E02D $01
        
        $06 $1D $E002 $00
        $FE $1D $E003 $01
        $06 $15 $E00A $00
        $FE $15 $E00B $01
        $06 $0D $E012 $00
        $FE $0D $E013 $01
    }


    ;#Data w vrtan.body.walk.r.0{
        $09
        $00 $08 $202A $00
        $08 $08 $202B $00
        $FF $10 $202C $01
        $07 $10 $202D $00
        
        $00 $00 $2002 $00
        $08 $00 $2003 $00
        $00 $08 $200A $00
        $08 $08 $200B $00
        $00 $10 $2012 $00
        $08 $10 $2013 $00
    }
    ;#Data w vrtan.body.walk.ri.0{
        $09
        $00 $15 $A02A $00
        $08 $15 $A02B $00
        $FF $0D $A02C $01
        $07 $0D $A02D $00
        
        $00 $1D $A002 $00
        $08 $1D $A003 $00
        $00 $15 $A00A $00
        $08 $15 $A00B $00
        $00 $0D $A012 $00
        $08 $0D $A013 $00
    }
    ;#Data w vrtan.body.walk.l.0{
        $09
        $06 $08 $602A $00
        $FE $08 $602B $01
        $07 $10 $602C $00
        $FF $10 $602D $01
        
        $06 $00 $6002 $00
        $FE $00 $6003 $01
        $06 $08 $600A $00
        $FE $08 $600B $01
        $06 $10 $6012 $00
        $FE $10 $6013 $01
    }
    ;#Data w vrtan.body.walk.li.0{
        $09
        $06 $15 $E02A $00
        $FE $15 $E02B $01
        $07 $0D $E02C $00
        $FF $0D $E02D $01
        
        $06 $1D $E002 $00
        $FE $1D $E003 $01
        $06 $15 $E00A $00
        $FE $15 $E00B $01
        $06 $0D $E012 $00
        $FE $0D $E013 $01
    }
    ;#Data w vrtan.body.walk.r.1{
        $09
        $00 $08 $202A $00
        $08 $08 $202B $00
        $FF $10 $202C $01
        $07 $10 $202D $00
        
        $00 $00 $2002 $00
        $08 $00 $2003 $00
        $00 $08 $2008 $00
        $08 $08 $2009 $00
        $00 $10 $2010 $00
        $08 $10 $2011 $00
    }
    ;#Data w vrtan.body.walk.ri.1{
        $09
        $00 $15 $A02A $00
        $08 $15 $A02B $00
        $FF $0D $A02C $01
        $07 $0D $A02D $00
        
        $00 $1D $A002 $00
        $08 $1D $A003 $00
        $00 $15 $A008 $00
        $08 $15 $A009 $00
        $00 $0D $A010 $00
        $08 $0D $A011 $00
    }
    ;#Data w vrtan.body.walk.l.1{
        $09
        $06 $08 $602A $00
        $FE $08 $602B $01
        $07 $10 $602C $00
        $FF $10 $602D $01
        
        $06 $00 $6002 $00
        $FE $00 $6003 $01
        $06 $08 $6008 $00
        $FE $08 $6009 $01
        $06 $10 $6010 $00
        $FE $10 $6011 $01
    }
    ;#Data w vrtan.body.walk.li.1{
        $09
        $06 $15 $E02A $00
        $FE $15 $E02B $01
        $07 $0D $E02C $00
        $FF $0D $E02D $01
        
        $06 $1D $E002 $00
        $FE $1D $E003 $01
        $06 $15 $E008 $00
        $FE $15 $E009 $01
        $06 $0D $E010 $00
        $FE $0D $E011 $01
    }
    ;#Data w vrtan.body.walk.r.2{
        $09
        $00 $08 $202A $00
        $08 $08 $202B $00
        $FF $10 $202C $01
        $07 $10 $202D $00
        
        $00 $00 $2000 $00
        $08 $00 $2001 $00
        $00 $08 $2006 $00
        $08 $08 $2007 $00
        $00 $10 $200E $00
        $08 $10 $200F $00
    }
    ;#Data w vrtan.body.walk.ri.2{
        $09
        $00 $15 $A02A $00
        $08 $15 $A02B $00
        $FF $0D $A02C $01
        $07 $0D $A02D $00
        
        $00 $1D $A000 $00
        $08 $1D $A001 $00
        $00 $15 $A006 $00
        $08 $15 $A007 $00
        $00 $0D $A00E $00
        $08 $0D $A00F $00
    }
    ;#Data w vrtan.body.walk.l.2{
        $09
        $06 $08 $602A $00
        $FE $08 $602B $01
        $07 $10 $602C $00
        $FF $10 $602D $01
        
        $06 $00 $6000 $00
        $FE $00 $6001 $01
        $06 $08 $6006 $00
        $FE $08 $6007 $01
        $06 $10 $600E $00
        $FE $10 $600F $01
    }
    ;#Data w vrtan.body.walk.li.2{
        $09
        $06 $15 $E02A $00
        $FE $15 $E02B $01
        $07 $0D $E02C $00
        $FF $0D $E02D $01
        
        $06 $1D $E000 $00
        $FE $1D $E001 $01
        $06 $15 $E006 $00
        $FE $15 $E007 $01
        $06 $0D $E00E $00
        $FE $0D $E00F $01
    }


    ;#Data w vrtan.body.jump.r.0{
        $09
        $01 $07 $202A $00
        $09 $07 $202B $00
        $00 $0F $202C $00
        $08 $0F $202D $00
        
        $01 $00 $2004 $00
        $09 $00 $2005 $00
        $01 $08 $200C $00
        $09 $08 $200D $00
        $00 $10 $2014 $00
        $08 $10 $2015 $00
    }
    ;#Data w vrtan.body.jump.ri.0{
        $09
        $01 $16 $A02A $00
        $09 $16 $A02B $00
        $00 $0E $A02C $00
        $08 $0E $A02D $00
        
        $01 $1D $A004 $00
        $09 $1D $A005 $00
        $01 $15 $A00C $00
        $09 $15 $A00D $00
        $00 $0D $A014 $00
        $08 $0D $A015 $00
    }
    ;#Data w vrtan.body.jump.l.0{
        $09
        $05 $07 $602A $00
        $FD $07 $602B $01
        $06 $0F $602C $00
        $FE $0F $602D $01
        
        $05 $00 $6004 $00
        $FD $00 $6005 $01
        $05 $08 $600C $00
        $FD $08 $600D $01
        $06 $10 $6014 $00
        $FE $10 $6015 $01
    }
    ;#Data w vrtan.body.jump.li.0{
        $09
        $05 $16 $E02A $00
        $FD $16 $E02B $01
        $06 $0E $E02C $00
        $FE $0E $E02D $01
        
        $05 $1D $E004 $00
        $FD $1D $E005 $01
        $05 $15 $E00C $00
        $FD $15 $E00D $01
        $06 $0D $E014 $00
        $FE $0D $E015 $01
    }


    ;#Data w vrtan.body.punch.r.0{
        $0A
        $00 $08 $202E $00
        $08 $08 $202F $00
        $10 $08 $2030 $00
        $00 $0F $2031 $00
        $08 $10 $2032 $00
        
        $00 $00 $2004 $00
        $08 $00 $2005 $00
        $00 $08 $200C $00
        $08 $08 $200D $00
        $00 $10 $2014 $00
        $08 $10 $2015 $00
    }
    ;#Data w vrtan.body.punch.ri.0{
        $0A
        $00 $15 $A02E $00
        $08 $15 $A02F $00
        $10 $15 $A030 $00
        $00 $0E $A031 $00
        $08 $0D $A032 $00
        
        $00 $1D $A004 $00
        $08 $1D $A005 $00
        $00 $15 $A00C $00
        $08 $15 $A00D $00
        $00 $0D $A014 $00
        $08 $0D $A015 $00
    }
    ;#Data w vrtan.body.punch.l.0{
        $0A
        $06 $08 $602E $00
        $FE $08 $602F $01
        $F6 $08 $6030 $01
        $06 $0F $6031 $00
        $FE $10 $6032 $01
        
        $06 $00 $6004 $00
        $FE $00 $6005 $01
        $06 $08 $600C $00
        $FE $08 $600D $01
        $06 $10 $6014 $00
        $FE $10 $6015 $01
    }
    ;#Data w vrtan.body.punch.li.0{
        $0A
        $06 $15 $E02E $00
        $FE $15 $E02F $01
        $F6 $15 $E030 $01
        $06 $0E $E031 $00
        $FE $0D $E032 $01
        
        $06 $1D $E004 $00
        $FE $1D $E005 $01
        $06 $15 $E00C $00
        $FE $15 $E00D $01
        $06 $0D $E014 $00
        $FE $0D $E015 $01
    }
    ;#Data w vrtan.body.punch.r.1{
        $0A
        $01 $08 $2033 $00
        $09 $08 $2034 $00
        $11 $08 $2035 $00
        $01 $10 $2036 $00
        $09 $10 $2037 $00
        
        $01 $00 $2004 $00
        $09 $00 $2005 $00
        $01 $08 $200C $00
        $09 $08 $200D $00
        $00 $10 $2016 $00
        $08 $10 $2017 $00
    }
    ;#Data w vrtan.body.punch.ri.1{
        $0A
        $01 $15 $A033 $00
        $09 $15 $A034 $00
        $11 $15 $A035 $00
        $01 $0D $A036 $00
        $09 $0D $A037 $00
        
        $01 $1D $A004 $00
        $09 $1D $A005 $00
        $01 $15 $A00C $00
        $09 $15 $A00D $00
        $00 $0D $A016 $00
        $08 $0D $A017 $00
    }
    ;#Data w vrtan.body.punch.l.1{
        $0A
        $05 $08 $6033 $00
        $FD $08 $6034 $01
        $F5 $08 $6035 $01
        $05 $10 $6036 $00
        $FD $10 $6037 $00
        
        $05 $00 $6004 $00
        $FD $00 $6005 $01
        $05 $08 $600C $00
        $FD $08 $600D $01
        $06 $10 $6016 $00
        $FE $10 $6017 $01
    }
    ;#Data w vrtan.body.punch.li.1{
        $0A
        $05 $15 $E033 $00
        $FD $15 $E034 $01
        $F5 $15 $E035 $01
        $05 $0D $E036 $00
        $FD $0D $E037 $00
        
        $05 $1D $E004 $00
        $FD $1D $E005 $01
        $05 $15 $E00C $00
        $FD $15 $E00D $01
        $06 $0D $E016 $00
        $FE $0D $E017 $01
    }


    ;#Data w vrtan.body.uppercut.r.0{
        $09
        $00 $06 $2038 $00
        $08 $06 $2039 $00
        $00 $0E $203A $00
        $08 $0E $203B $00
        
        $00 $00 $2002 $00
        $08 $00 $2003 $00
        $00 $08 $2008 $00
        $08 $08 $2009 $00
        $00 $10 $2010 $00
        $08 $10 $2011 $00
    }
    ;#Data w vrtan.body.uppercut.ri.0{
        $09
        $00 $17 $A038 $00
        $08 $17 $A039 $00
        $00 $0F $A03A $00
        $08 $0F $A03B $00
        
        $00 $1D $A002 $00
        $08 $1D $A003 $00
        $00 $15 $A008 $00
        $08 $15 $A009 $00
        $00 $0D $A010 $00
        $08 $0D $A011 $00
    }
    ;#Data w vrtan.body.uppercut.l.0{
        $09
        $06 $06 $6038 $00
        $FE $06 $6039 $01
        $06 $0E $603A $00
        $FE $0E $603B $01
        
        $06 $00 $6002 $00
        $FE $00 $6003 $01
        $06 $08 $6008 $00
        $FE $08 $6009 $01
        $06 $10 $6010 $00
        $FE $10 $6011 $01
    }
    ;#Data w vrtan.body.uppercut.li.0{
        $09
        $06 $17 $E038 $00
        $FE $17 $E039 $01
        $06 $0F $E03A $00
        $FE $0F $E03B $01
        
        $06 $1D $E002 $00
        $FE $1D $E003 $01
        $06 $15 $E008 $00
        $FE $15 $E009 $01
        $06 $0D $E010 $00
        $FE $0D $E011 $01
    }
    ;#Data w vrtan.body.uppercut.r.1{
        $0B
        $00 $01 $203C $00
        $08 $01 $203D $00
        $00 $09 $203E $00
        $08 $09 $203F $00
        $00 $11 $2040 $00
        $08 $11 $2041 $00
        
        $00 $00 $2000 $00
        $08 $00 $2001 $00
        $00 $08 $2006 $00
        $08 $08 $2007 $00
        $00 $10 $200E $00
        $08 $10 $200F $00
    }
    ;#Data w vrtan.body.uppercut.ri.1{
        $0B
        $00 $1C $A03C $00
        $08 $1C $A03D $00
        $00 $14 $A03E $00
        $08 $14 $A03F $00
        $00 $0C $A040 $00
        $08 $0C $A041 $00
        
        $00 $1D $A000 $00
        $08 $1D $A001 $00
        $00 $15 $A006 $00
        $08 $15 $A007 $00
        $00 $0D $A00E $00
        $08 $0D $A00F $00
    }
    ;#Data w vrtan.body.uppercut.l.1{
        $0B
        $06 $01 $603C $00
        $FE $01 $603D $01
        $06 $09 $603E $00
        $FE $09 $603F $01
        $06 $11 $6040 $00
        $FE $11 $6041 $01
        
        $06 $00 $6000 $00
        $FE $00 $6001 $01
        $06 $08 $6006 $00
        $FE $08 $6007 $01
        $06 $10 $600E $00
        $FE $10 $600F $01
    }
    ;#Data w vrtan.body.uppercut.li.1{
        $0B
        $06 $1C $E03C $00
        $FE $1C $E03D $01
        $06 $14 $E03E $00
        $FE $14 $E03F $01
        $06 $0C $E040 $00
        $FE $0C $E041 $01
        
        $06 $1D $E000 $00
        $FE $1D $E001 $01
        $06 $15 $E006 $00
        $FE $15 $E007 $01
        $06 $0D $E00E $00
        $FE $0D $E00F $01
    }
    ;#Data w vrtan.body.uppercut.r.2{
        $0B
        $00 $00 $203C $00
        $08 $00 $203D $00
        $00 $08 $203E $00
        $08 $08 $203F $00
        $00 $10 $2040 $00
        $08 $10 $2041 $00
        
        $00 $00 $2000 $00
        $08 $00 $2001 $00
        $00 $08 $2006 $00
        $08 $08 $2007 $00
        $00 $10 $200E $00
        $08 $10 $200F $00
    }
    ;#Data w vrtan.body.uppercut.ri.2{
        $0B
        $00 $1D $A03C $00
        $08 $1D $A03D $00
        $00 $15 $A03E $00
        $08 $15 $A03F $00
        $00 $0D $A040 $00
        $08 $0D $A041 $00
        
        $00 $1D $A000 $00
        $08 $1D $A001 $00
        $00 $15 $A006 $00
        $08 $15 $A007 $00
        $00 $0D $A00E $00
        $08 $0D $A00F $00
    }
    ;#Data w vrtan.body.uppercut.l.2{
        $0B
        $06 $00 $603C $00
        $FE $00 $603D $01
        $06 $08 $603E $00
        $FE $08 $603F $01
        $06 $10 $6040 $00
        $FE $10 $6041 $01
        
        $06 $00 $6000 $00
        $FE $00 $6001 $01
        $06 $08 $6006 $00
        $FE $08 $6007 $01
        $06 $10 $600E $00
        $FE $10 $600F $01
    }
    ;#Data w vrtan.body.uppercut.li.2{
        $0B
        $06 $1D $E03C $00
        $FE $1D $E03D $01
        $06 $15 $E03E $00
        $FE $15 $E03F $01
        $06 $0D $E040 $00
        $FE $0D $E041 $01
        
        $06 $1D $E000 $00
        $FE $1D $E001 $01
        $06 $15 $E006 $00
        $FE $15 $E007 $01
        $06 $0D $E00E $00
        $FE $0D $E00F $01
    }
    
	;#Data w vrtan.body.crush.r.0{
	    $0A
		$03 $00 $2042 $00
		$0B $00 $2043 $00
		$FB $08 $2044 $01
		$03 $08 $2045 $00
		$0B $08 $2046 $00
		$FB $10 $2047 $01
		$03 $10 $2048 $00
		$0B $10 $2049 $00
		$FB $18 $204A $01
		$03 $18 $204B $00
		$0B $18 $204C $00
	}
	;#Data w vrtan.body.crush.ri.0{
	    $0A
		$03 $1D $A042 $00
		$0B $1D $A043 $00
		$FB $15 $A044 $01
		$03 $15 $A045 $00
		$0B $15 $A046 $00
		$FB $0D $A047 $01
		$03 $0D $A048 $00
		$0B $0D $A049 $00
		$FB $05 $A04A $01
		$03 $05 $A04B $00
		$0B $05 $A04C $00
	}
	;#Data w vrtan.body.crush.l.0{
	    $0A
		$03 $00 $6042 $00
		$FB $00 $6043 $01
		$0B $08 $6044 $00
		$03 $08 $6045 $00
		$FB $08 $6046 $01
		$0B $10 $6047 $00
		$03 $10 $6048 $00
		$FB $10 $6049 $01
		$0B $18 $604A $00
		$03 $18 $604B $00
		$FB $18 $604C $01
	}
	;#Data w vrtan.body.crush.li.0{
	    $0A
		$03 $1D $E042 $00
		$FB $1D $E043 $01
		$0B $15 $E044 $00
		$03 $15 $E045 $00
		$FB $15 $E046 $01
		$0B $0D $E047 $00
		$03 $0D $E048 $00
		$FB $0D $E049 $01
		$0B $05 $E04A $00
		$03 $05 $E04B $00
		$FB $05 $E04C $01
	}
    
	;#Data w vrtan.body.crush.r.1{
	    $07
		$03 $08 $204D $00
		$0B $08 $204E $00
		$FB $10 $204F $01
		$03 $10 $2050 $00
		$0B $10 $2051 $00
		$FB $18 $2052 $01
		$03 $18 $2053 $00
		$0B $18 $2054 $00
	}
	;#Data w vrtan.body.crush.ri.1{
	    $07
		$03 $15 $A04D $00
		$0B $15 $A04E $00
		$FB $0D $A04F $01
		$03 $0D $A050 $00
		$0B $0D $A051 $00
		$FB $05 $A052 $01
		$03 $05 $A053 $00
		$0B $05 $A054 $00
	}
	;#Data w vrtan.body.crush.l.1{
	    $07
		$03 $08 $604D $00
		$FB $08 $604E $01
		$0B $10 $604F $00
		$03 $10 $6050 $00
		$FB $10 $6051 $01
		$0B $18 $6052 $00
		$03 $18 $6053 $00
		$FB $18 $6054 $01
	}
	;#Data w vrtan.body.crush.li.1{
	    $07
		$03 $15 $E04D $00
		$FB $15 $E04E $01
		$0B $0D $E04F $00
		$03 $0D $E050 $00
		$FB $0D $E051 $01
		$0B $05 $E052 $00
		$03 $05 $E053 $00
		$FB $05 $E054 $01
	}
	
	;#Data w vrtan.body.crush.r.2{
	    $0B
		$F7 $0C $3068 $01
		$FF $0C $3069 $01
		$07 $0C $306A $00
		$0F $0C $306B $00
		$F7 $14 $306C $01
		$FF $14 $306D $01
		$07 $14 $306E $00
		$0F $14 $306F $00
		$F7 $1C $3070 $01
		$FF $1C $3071 $01
		$07 $1C $3072 $00
		$0F $1C $3073 $00
	}
	;#Data w vrtan.body.crush.ri.2{
	    $0B
		$F7 $11 $B068 $01
		$FF $11 $B069 $01
		$07 $11 $B06A $00
		$0F $11 $B06B $00
		$F7 $09 $B06C $01
		$FF $09 $B06D $01
		$07 $09 $B06E $00
		$0F $09 $B06F $00
		$F7 $01 $B070 $01
		$FF $01 $B071 $01
		$07 $01 $B072 $00
		$0F $01 $B073 $00
	}
	;#Data w vrtan.body.crush.l.2{
	    $0B
		$0F $0C $7068 $00
		$07 $0C $7069 $00
		$FF $0C $706A $01
		$F7 $0C $706B $01
		$0F $14 $706C $00
		$07 $14 $706D $00
		$FF $14 $706E $01
		$F7 $14 $706F $01
		$0F $1C $7070 $00
		$07 $1C $7071 $00
		$FF $1C $7072 $01
		$F7 $1C $7073 $01
	}
	;#Data w vrtan.body.crush.li.2{
	    $0B
		$0F $11 $F068 $00
		$07 $11 $F069 $00
		$FF $11 $F06A $01
		$F7 $11 $F06B $01
		$0F $09 $F06C $00
		$07 $09 $F06D $00
		$FF $09 $F06E $01
		$F7 $09 $F06F $01
		$0F $01 $F070 $00
		$07 $01 $F071 $00
		$FF $01 $F072 $01
		$F7 $01 $F073 $01
	}


    ;# ===============================================================//
    ;# Legs Sprites                                                   //
    ;# ===============================================================//
    ;#Data w vrtan.legs.idle.r.0 {
        $01
        $00 $18 $2018 $00
        $08 $18 $2019 $00
    }
    ;#Data w vrtan.legs.idle.ri.0 {
        $01
        $00 $05 $A018 $00
        $08 $05 $A019 $00
    }
    ;#Data w vrtan.legs.idle.l.0 {
        $01
        $06 $18 $6018 $00
        $FE $18 $6019 $01
    }
    ;#Data w vrtan.legs.idle.li.0 {
        $01
        $06 $05 $E018 $00
        $FE $05 $E019 $01
    }


    ;#Data w vrtan.legs.walk.r.0 {
        $01
        $00 $18 $201A $00
        $08 $18 $201B $00
    }
    ;#Data w vrtan.legs.walk.ri.0 {
        $01
        $00 $05 $A01A $00
        $08 $05 $A01B $00
    }
    ;#Data w vrtan.legs.walk.l.0 {
        $01
        $06 $18 $601A $00
        $FE $18 $601B $01
    }
    ;#Data w vrtan.legs.walk.li.0 {
        $01
        $06 $05 $E01A $00
        $FE $05 $E01B $01
    }
    ;#Data w vrtan.legs.walk.r.1 {
        $01
        $00 $18 $201C $00
        $08 $18 $201D $00
    }
    ;#Data w vrtan.legs.walk.ri.1 {
        $01
        $00 $05 $A01C $00
        $08 $05 $A01D $00
    }
    ;#Data w vrtan.legs.walk.l.1 {
        $01
        $06 $18 $601C $00
        $FE $18 $601D $01
    }
    ;#Data w vrtan.legs.walk.li.1 {
        $01
        $06 $05 $E01C $00
        $FE $05 $E01D $01
    }
    ;#Data w vrtan.legs.walk.r.2 {
        $00
        $04 $18 $201E $00
    }
    ;#Data w vrtan.legs.walk.ri.2 {
        $00
        $04 $05 $A01E $00
    }
    ;#Data w vrtan.legs.walk.l.2 {
        $00
        $04 $18 $201E $00
    }
    ;#Data w vrtan.legs.walk.li.2 {
        $00
        $04 $05 $A01E $00
    }
    ;#Data w vrtan.legs.walk.r.3 {
        $01
        $00 $18 $2020 $00
        $08 $18 $2021 $00
    }
    ;#Data w vrtan.legs.walk.ri.3 {
        $01
        $00 $05 $A020 $00
        $08 $05 $A021 $00
    }
    ;#Data w vrtan.legs.walk.l.3 {
        $01
        $06 $18 $6020 $00
        $FE $18 $6021 $01
    }
    ;#Data w vrtan.legs.walk.li.3 {
        $01
        $06 $05 $E020 $00
        $FE $05 $E021 $01
    }
    ;#Data w vrtan.legs.walk.r.4 {
        $01
        $00 $18 $2022 $00
        $08 $18 $2023 $00
    }
    ;#Data w vrtan.legs.walk.ri.4 {
        $01
        $00 $05 $A022 $00
        $08 $05 $A023 $00
    }
    ;#Data w vrtan.legs.walk.l.4 {
        $01
        $06 $18 $6022 $00
        $FE $18 $6023 $01
    }
    ;#Data w vrtan.legs.walk.li.4 {
        $01
        $06 $05 $E022 $00
        $FE $05 $E023 $01
    }
    ;#Data w vrtan.legs.walk.r.5 {
        $00
        $04 $18 $201F $00
    }
    ;#Data w vrtan.legs.walk.ri.5 {
        $00
        $04 $05 $A01F $00
    }
    ;#Data w vrtan.legs.walk.l.5 {
        $00
        $04 $18 $601F $00
    }
    ;#Data w vrtan.legs.walk.li.5 {
        $00
        $04 $05 $A01F $00
    }


    ;#Data w vrtan.legs.jump.r.0 {
        $01
        $00 $18 $2024 $00
        $08 $18 $2025 $00
    }
    ;#Data w vrtan.legs.jump.ri.0 {
        $01
        $00 $05 $A024 $00
        $08 $05 $A025 $00
    }
    ;#Data w vrtan.legs.jump.l.0 {
        $01
        $06 $18 $6024 $00
        $FE $18 $6025 $01
    }
    ;#Data w vrtan.legs.jump.li.0 {
        $01
        $06 $05 $E024 $00
        $FE $05 $E025 $01
    }
    
    
    ;# ===============================================================//
    ;#                                                                //
    ;# ===============================================================//

